<div>
    ...
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-lg font-semibold text-slate-900 dark:text-white">Expense Categories</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Used to group both one-off expenses and recurring schedules.</p>
        </div>
        @can('expenses.create')
            ...
            <tbody class="divide-y divide-slate-100 bg-white dark:divide-slate-700 dark:bg-slate-900">
                @forelse ($categories as $category)
                    <tr wire:key="cat-{{ $category->id }}">
                        <td class="px-4 py-2.5 text-sm font-medium text-slate-800 dark:text-slate-100">{{ $category->name }}</td>
                        <td class="px-4 py-2.5 text-sm text-slate-500 dark:text-slate-400">{{ $category->description ?: '—' }}</td>
                        <td class="px-4 py-2.5 text-right text-sm">
                            @can('expenses.create')
                                <button wire:click="edit({{ $category->id }})" class="font-medium text-sky-600 hover:text-sky-800 dark:text-sky-400">Edit</button>
                            @endcan
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="3" class="px-4 py-8 text-center text-sm text-slate-400">No expense categories yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <x-drawer :show="$showDrawer" :title="$editing ? 'Edit category' : 'New category'">
